...
function ShopPage() {
  ...
  return <section className="content-section page-section shop-page">
    <div className="page-title split">
      <div><span className="section-kicker">RANDOM BOX SHOP</span><h1>랜투샵</h1><p>카테고리별 랜덤박스를 비교하고 원하는 박스를 선택해 보세요.</p></div>
...
function HomePage() {
  ...
  return <>
    ...
    <section className="flow-section"><div className="section-heading"><div><span className="section-kicker">HOW IT WORKS</span><h2>열고, 고르고, 다시 연결되는 흐름</h2></div></div><div className="flow-grid">{[[ShoppingBag,'01','박스 구매','가격과 구성·확률을 확인하고 박스를 구매해요.'],[Sparkles,'02','몰입형 개봉','웹 네이티브 GSAP 무대에서 터치로 결과를 확인해요.'],[WalletCards,'03','상품 선택','보관, 트레이드, 배송, 포인트 전환 중 선택해요.'],[Truck,'04','수령과 순환','원하는 상품은 배송받고 나머지는 다시 순환시켜요.']].map(([Icon,no,title,desc]:any) => <article key={no}><div><Icon size={20}/><span>{no}</span></div><h3>{title}</h3><p>{desc}</p></article>)}</div></section>
  </>;
}
