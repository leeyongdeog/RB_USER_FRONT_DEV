...
function HomePage() {
  ...
  return <>
    <section className="hero hero-with-banner"><div className="hero-copy hero-copy-with-visual"><div className="hero-copy-backdrop" aria-hidden="true"><div className="hero-ring one"/><div className="hero-ring two"/>{featured && <GiftVisual box={featured} large/>}<div className="floating-label top"><b>{featured?.typeName || 'BOX'}</b><span>대표 박스</span></div><div className="floating-label bottom"><b>{onSaleCount}</b><span>현재 판매 중</span></div></div><span className="section-kicker"><Sparkles size={14}/> 오늘의 드롭</span><h1>결과보다 먼저,<br/><em>기대감</em>을 열어보세요.</h1><p>원하는 테마의 박스를 고르고, 화면을 터치하는 순간 시작되는 특별한 개봉 경험.</p><div className="hero-actions"><Link to={featured ? `/boxes/${featured.slug}` : '#box-collection'}>지금 열어보기 <ArrowRight size={17}/></Link><Link to="/inventory" className="ghost">내 인벤토리</Link></div><div className="trust-row"><span><ShieldCheck size={16}/> 확률 버전 공개</span><span><PackageCheck size={16}/> 실물 배송</span><span><ArrowLeftRight size={16}/> 안전한 트레이드</span></div></div><div className="hero-banner-stage"><BannerCarousel placement="HOME" fullWidth/></div></section>
