...
import {
  ArrowLeftRight, ArrowRight, Bell, Box, ChevronDown, ChevronLeft, ChevronRight, Circle, CircleHelp, Clock3,
...
function HomeBoxCarousel({ boxes }: { boxes: ApiBox[] }) {
  const pageSize = 3;
  const pageCount = Math.max(1, Math.ceil(boxes.length / pageSize));
  const pages = Array.from({ length: pageCount }, (_, pageIndex) => boxes.slice(pageIndex * pageSize, (pageIndex + 1) * pageSize));
  ...
  useEffect(() => setPage(0), [boxes]);
  useEffect(() => {
    if (!desktop || paused || pageCount <= 1) return;
    const timer = window.setInterval(() => setPage(current => (current + 1) % pageCount), 4500);
    ...
    <div className="home-box-carousel-viewport">
      <div className="home-box-carousel-track" style={{ '--carousel-page': page } as React.CSSProperties}>
        {pages.map((pageBoxes, pageIndex) => <div className="home-box-carousel-page" key={`page-${pageIndex}`}>
          {pageBoxes.map((box, boxIndex) => <BoxCard key={box.id} box={box} featured={pageIndex === 0 && boxIndex === 0}/>) }
        </div>)}
      </div>
    </div>
    {pageCount > 1 && <>
      <button type="button" className="home-box-carousel-arrow prev" onClick={() => move(-1)} aria-label="이전 박스 목록"><ChevronLeft size={22}/></button>
      <button type="button" className="home-box-carousel-arrow next" onClick={() => move(1)} aria-label="다음 박스 목록"><ChevronRight size={22}/></button>
      <div className="home-box-carousel-dots" aria-label="박스 목록 페이지">{Array.from({length:pageCount},(_,index)=><button type="button" key={index} className={index===page?'active':''} onClick={()=>setPage(index)} aria-label={`${index+1}페이지`}/>)}</div>
    </>}
  </div>;
}
...
function HomePage() {
  ...
  return <>
    <section className="home-banner-stage"><BannerCarousel placement="HOME" fullWidth/></section>
    <section className="home-catalog-layout"><div className="hero-copy hero-copy-with-visual home-drop-card"><div className="hero-copy-backdrop" aria-hidden="true"><div className="hero-ring one"/><div className="hero-ring two"/>{featured && <GiftVisual box={featured} large/>}</div><span className="section-kicker"><Sparkles size={14}/> 오늘의 드롭</span><h1>결과보다 먼저,<br/><em>기대감</em>을 열어보세요.</h1><p>원하는 테마의 박스를 고르고, 화면을 터치하는 순간 시작되는 특별한 개봉 경험.</p><div className="hero-actions"><Link to={featured ? `/boxes/${featured.slug}` : '#box-collection'}>지금 열어보기 <ArrowRight size={17}/></Link><Link to="/inventory" className="ghost">내 인벤토리</Link></div><div className="trust-row"><span><ShieldCheck size={16}/> 확률 버전 공개</span><span><PackageCheck size={16}/> 실물 배송</span><span><ArrowLeftRight size={16}/> 안전한 트레이드</span></div></div><section className="content-section home-box-collection" id="box-collection"><div className="section-heading"><div><span className="section-kicker">BOX COLLECTION</span><h2>어떤 행운을 열어볼까요?</h2><p>K-POP 굿즈, K-뷰티, K-FOOD 카테고리에서 원하는 박스를 선택하세요.</p></div><label className="box-selector"><span className="sr-only">박스 카테고리</span><select value={category} onChange={event => setCategory(event.target.value)}><option value="ALL">전체 카테고리</option>{categories.map(item => <option key={item.code} value={item.code}>{item.name}</option>)}</select><ChevronDown size={15}/></label></div><CatalogCategoryTabs categories={categories} selected={category} onChange={setCategory}/>{isLoading ? <div className="data-state">박스 목록을 불러오고 있습니다.</div> : isError ? <div className="data-state error">박스 목록을 불러오지 못했습니다.</div> : filteredBoxes.length ? <HomeBoxCarousel boxes={filteredBoxes}/> : <div className="data-state">선택한 카테고리의 판매 박스가 없습니다.</div>}</section></section>
