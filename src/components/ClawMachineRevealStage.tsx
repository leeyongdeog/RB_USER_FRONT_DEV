...
export default function ClawMachineRevealStage({
  ...
  const [openClickCount, setOpenClickCount] = useState(0);
  const [backdropColor, setBackdropColor] = useState('#071124');
  const [hasChargedBackdrop, setHasChargedBackdrop] = useState(false);
  ...
  const resultColor = kind === 'single' ? '#ffffff' : kind === 'double' ? '#58dfff' : kind === 'level-up' ? '#ff4f63' : '#ffd45a';
  const capsulePalette = { top: '#fff0a0', bottom: '#c77b08' };
  ...
  const startReveal = () => {
    ...
    setStarted(true);
    setOpenClickCount(0);
    setBackdropColor('#071124');
    setHasChargedBackdrop(false);
  ...
  const openPrize = () => {
    ...
    const nextClickCount = openClickCount + 1;
    const clickColor = kind === 'single'
      ? '#ffffff'
      : nextClickCount === 1
      ? '#ffffff'
      : nextClickCount === 2 && (kind === 'level-up' || kind === 'jackpot')
        ? '#ff4f63'
        : nextClickCount === 3 && kind === 'double'
          ? '#58dfff'
          : nextClickCount === 3 && kind === 'level-up'
            ? '#ff4f63'
            : nextClickCount === 3 && kind === 'jackpot'
              ? '#ffd45a'
              : '#ffffff';
    setBackdropColor(clickColor);
    setHasChargedBackdrop(true);
    stageRef.current?.style.setProperty('--claw-backdrop-color', clickColor);
    runningRef.current = true;
    setOpenClickCount(nextClickCount);
    playSound(clickAudioRef.current);

    if (nextClickCount < 3) {
      seamFxRef.current?.charge({ color: clickColor, duration: .36, decayDuration: .432 });
      const leftEye = prizeRef.current?.querySelector<HTMLElement>(':scope > .eye.left');
      const rightEye = prizeRef.current?.querySelector<HTMLElement>(':scope > .eye.right');
      const eyeTargets = [leftEye, rightEye].filter(Boolean);
      gsap.timeline()
        .set(eyeTargets, {
          animation: 'none', width: 25, height: 25, border: 0, borderRadius: 2,
          background: 'transparent', boxShadow: 'none', opacity: 0,
          transformOrigin: '50% 50%', scale: .35,
        })
        .set(leftEye || [], { borderRight: '5px solid #13203b', borderBottom: '5px solid #13203b', rotation: -45 })
        .set(rightEye || [], { borderLeft: '5px solid #13203b', borderBottom: '5px solid #13203b', rotation: 45 })
        .to(eyeTargets, { opacity: 1, scale: 1, duration: .065, ease: 'back.out(3)' })
        .to({}, { duration: .5 })
        .to(eyeTargets, { opacity: 0, scale: .35, duration: .13, ease: 'power2.in' })
        .set(eyeTargets, { clearProps: 'all' });
      gsap.timeline({
        onComplete: () => { runningRef.current = false; },
      })
        .to(prizeRef.current, { scaleX: .78, scaleY: 1.18, duration: .07, ease: 'power3.in' })
        .to(prizeRef.current, { scaleX: 1.18, scaleY: .82, duration: .09, ease: 'power3.out' })
        .to(prizeRef.current, { scaleX: .9, scaleY: 1.12, duration: .08, ease: 'sine.inOut' })
        .to(prizeRef.current, { scaleX: 1.05, scaleY: .94, duration: .08, ease: 'sine.inOut' })
        .to(prizeRef.current, { scaleX: 1, scaleY: 1, duration: .12, ease: 'elastic.out(1,.42)' })
        .to({}, { duration: .352 });
      return;
    }
    ...
    seamFxRef.current?.charge({ color: clickColor, duration: 2.04 });
  ...
  return <div
    ref={stageRef}
    className={`preview-stage alternative-reveal-stage claw-reveal-stage quality-${quality} result-${resultClass}${started ? ' is-started' : ''}${readyToOpen ? ' is-awaiting-open' : ''}${hasChargedBackdrop ? ' has-charged-backdrop' : ''}${opening ? ' is-opening' : ''}${revealed ? ' is-revealed' : ''}`}
    style={{
      '--claw-result-color': resultColor,
      '--claw-backdrop-color': backdropColor,
  ...
  >
    ...
    {readyToOpen && <div className="claw-instruction claw-three-click-instruction"><b>황금 캡슐을 눌러 힘을 충전하세요</b><span>개봉까지 {3 - openClickCount}번 남았습니다.</span></div>}
