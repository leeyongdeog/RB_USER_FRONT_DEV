...
void main() {
  float expansionProgress = uExpandEnd < .999
    ? clamp(uProgress / max(uExpandEnd, .001), 0.0, 1.0)
    : uProgress;
  float expansion = smoothstep(.04, .74, expansionProgress);
...
void main() {
  ...
  float expansionProgress = uExpandEnd < .999
    ? clamp(uProgress / max(uExpandEnd, .001), 0.0, 1.0)
    : uProgress;
  float envelope = smoothstep(.02, .18, expansionProgress);
  float eraseProgress = uExpandEnd < .999
    ? smoothstep(uExpandEnd, 1.0, uProgress)
    : smoothstep(.48, 1.0, uProgress);
  float eraseFront = eraseProgress * 1.18 - .12;
  float innerOutFade = smoothstep(eraseFront, eraseFront + .1, vRayUv.x);
  float glowPulse = .9 + .1 * sin(vRayUv.x * 17.0 + vStrength * 8.0);
  float alpha = (outerGlow * .48 + middleGlow * .38 + softBody * .48 + brightCore * .96)
    * headFade * tailFade * envelope * innerOutFade * vStrength * glowPulse;
...
export class CapsuleSeamFxEngine {
  ...
  charge({ color, duration = 1.8, decayDuration }: CapsuleSeamFxOptions) {
    this.rayMaterial.uniforms.uColor.value.set(color);
    this.auraMaterial.uniforms.uColor.value.set(color);
    this.effectDuration = duration + (decayDuration ?? 0);
    this.rayMaterial.uniforms.uExpandEnd.value = decayDuration
